# EPIC Plan — AgentMonitor "Pixel Experience" v3 — zCorvus + pixel-agents-zcorvus + OpenCode (2026-03-26)

## Metadata
- **PlanId:** `aiw-epic-agentmonitor-pixel-experience-plan-v3-20260326`
- **Versión:** `3.2` — ENFOQUE WEBVIEW-STANDALONE (NO extensión VS Code)
- **Fecha:** 2026-03-27
- **Planner:** `Planner`
- **CorrelationId:** `aiw-agentmonitor-pixel-experience-20260326`
- **Estado:** `active`
- **Ubicación:** `AI_Workspace/pixel-agents-zcorvus/webview-ui/` (web standalone, NO extensión)
- **Herramienta de desarrollo:** OpenCode
- **Reemplaza:** v3.1 ( extensión), v3.0, v2, v1

---

## 0) Por qué se descarta el plan v1 y v2/v3.1
...
`pixel-agents` es una **extensión VS Code completa** con su propio motor de rendering, construida originalmente para Claude Code. El proyecto tiene dos partes:
1. **Extension host** (TypeScript + VS Code API) - lee JSONL de sesiones Claude Code
2. **Webview UI** (Vite + React + Canvas 2D) - renderiza oficina pixel con personajes animados

### La corrección de v3.2: Webview standalone, NO extensión
El usuario quiere **solo la parte visual** (webview-ui) como una aplicación web independiente, NO una extensión VS Code.

- Se usa el motor visual de pixel-agents (Canvas 2D + game loop)
- NO se usa la extensión VS Code
- La fuente de datos es el MCP de zCorvus (no JSONL de Claude Code)
- El resultado es una página web standalone que se puede integrar en AgentMonitor o abrir por separado
...
## 1) Arquitectura de integración zCorvus ↔ pixel-agents (webview standalone)

### 1.1 Arquitectura original de pixel-agents (VS Code Extension)
...
Extension Host (TypeScript)
  ...
  └─ MessageBridge → postMessage API
      ↓
Webview (Canvas)             ← SE REUTILIZA (standalone)
  └─ CharacterStateManager
  └─ PixelOfficeRenderer (BFS + game loop)
  └─ UI (agent cards, layout editor)
```

### 1.2 Arquitectura del fork zCorvus (WEBVIEW STANDALONE)
...
zCorvus MCP Server
  ...
  └─ HTTP GET /api/events      ← histórico / polling fallback
      ↓
[NUEVO] MCP Adapter (TypeScript/JS - frontend)
  └─ McpWebSocketClient        ← cliente WebSocket standalone
  └─ AgentStateMapper          ← eventos MCP → estados pixel-agents
  └─ AgentRegistry (reescrito) ← 5 agentes zCorvus fijos, sin terminales
      ↓
Webview UI standalone (Canvas)
  └─ CharacterStateManager (sin cambios)
  └─ PixelOfficeRenderer (sin cambios)
```

**NO hay extensión VS Code**. La webview se ejecuta como app standalone, consultando directamente el MCP de zCorvus.
...
1. **Eliminar lógica de VS Code/JSONL**: La webview standalone no necesita la extensión, solo el código del webview-ui
2. **MCP Adapter standalone**: cliente WebSocket + HTTP que consume `/ws` y `/api/events` de zCorvus
3. **AgentStateMapper**: convierte eventos MCP en estados del state machine de pixel-agents
4. **AgentRegistry reescrito**: inicializa los 5 agentes de zCorvus como personajes fijos al arrancar
5. **Configuración de layout**: preset de oficina con los 5 agentes en sus posiciones
6. **Indicadores visuales de estado crítico**: `TASK_BLOCKED` / `INCIDENT_OPENED` con alertas visuales

### 2.4 Lo que NO hay que hacer
- NO reescribir el motor de canvas
- NO crear extensión VS Code
- NO modificar el contrato MCP de zCorvus
- NO reemplazar el AgentMonitor web existente — es una **vista complementaria**
- NO usar JSONL de Claude Code

---

## 3) Principios de integración

1. **Webview standalone**: Solo se usa la parte visual de pixel-agents (`webview-ui`), NO la extensión VS Code
2. **Adapter-first**: el MCP Adapter es una capa de traducción en el frontend, no una extensión
3. **Estados explícitos > heurísticas**: los eventos MCP de zCorvus dan certeza de estado
4. **No romper MCP**: el adapter es un cliente pasivo (read-only) del MCP server
5. **Build incremental**: cada fase produce una app web funcional
6. **OpenCode como entorno de desarrollo**: la herramienta es OpenCode, no Claude Code
...
- **Descripción:** La copia local ya existe en `AI_Workspace/pixel-agents-zcorvus/`. Validar que la webview standalone compila y funciona. Eliminar código de la extensión VS Code que no se usa (JSONL watcher, terminal detection).
- **Pasos concretos:**
  ```bash
  cd AI_Workspace/pixel-agents-zcorvus/webview-ui
  npm install
  npm run build   # validar que la webview compila
  npm run dev     # probar en localhost
  ```
- **Acceptance criteria:**
  - Webview standalone compila sin errores.
  - Código de extensión VS Code innecesario identificado (opcional: eliminar).
  - Mapa de código documentado: qué se usa, qué se ignora, puntos de extensión para el adapter.
...
- **Descripción:** Verificar que la copia local está lista para desarrollo: JSONL watcher eliminado, build limpio, MCP auditado, puntos de extensión identificados.
- **Acceptance criteria:**
  - Checklist de readiness: copia local compila, JSONL watcher ausente, no hay referencias a Claude Code en el código activo.
...
- **Descripción:** Empaquetar la webview standalone como directorio estático desplegable.
- **Pasos:**
  ```bash
  cd AI_Workspace/pixel-agents-zcorvus/webview-ui
  npm run build
  # Output: dist/ con archivos estáticos
  ```
- **Acceptance criteria:**
  - Build genera archivos estáticos en `dist/`.
  - La webview se puede servir con cualquier servidor estático (nginx, etc).
...
- **assignedTo:** `Documenter`
- **dependsOn:** [`aiw-tester-pixel-v2-gate-f3-20260326-01`]
- **Descripción:** Publicar runbook operativo: cómo instalar dependencias, ejecutar, configurar URL del MCP server y hacer debug.
- **Acceptance criteria:**
  - Runbook cubre: instalación, configuración de URL MCP, troubleshooting, despliegue estático.
  - Checklist de "webview sana" para diagnóstico rápido.
  - Registry documental actualizado.
