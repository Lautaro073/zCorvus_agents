...
const ALERT_BLOCKED_COLOR = '#ef4444';
const ALERT_INCIDENT_COLOR = '#f59e0b';
const ALERT_SIZE_PX = 8;
...
export function renderAlerts(
  ...
  for (const ch of characters) {
    if (!ch.alertType) continue;

    const color = ch.alertType === 'blocked' ? ALERT_BLOCKED_COLOR : ALERT_INCIDENT_COLOR;
    const sittingOff = ch.state === CharacterState.TYPE ? CHARACTER_SITTING_OFFSET_PX : 0;
    const alertX = Math.round(offsetX + (ch.x + TILE_SIZE / 2 + 4) * zoom);
    const alertY = Math.round(offsetY + (ch.y + sittingOff - TILE_SIZE / 2 - 8) * zoom);

    ctx.save();
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.arc(alertX, alertY, ALERT_SIZE_PX * zoom / 2, 0, Math.PI * 2);
    ctx.fill();
    ctx.strokeStyle = '#fff';
    ctx.lineWidth = zoom;
    ctx.stroke();
...
export function renderFrame(
  ...
  // Alert indicators (blocked/incident)
  renderAlerts(ctx, characters, offsetX, offsetY, zoom);
