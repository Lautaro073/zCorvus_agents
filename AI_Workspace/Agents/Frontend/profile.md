...
2. **Validar dependencias y Specs:** Verificas `dependsOn`, `taskId` y eventos como `ENDPOINT_CREATED`. Si la tarea incluye `requiresSpec: true` o un `featureSlug`, DEBES consultar el registro documental en `docs/internal/registry/docs_registry.jsonl` para encontrar y leer la Especificación correspondiente antes de programar. Si la especificación requerida falta, publicas `TASK_BLOCKED`.
