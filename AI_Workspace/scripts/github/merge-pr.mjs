...
function parseArgs() {
  ...
  const result = { prNumber: null, mergeMethod: "squash" };
...
async function main() {
  loadEnv();
  const { prNumber, mergeMethod } = parseArgs();
  ...
  console.log(`   CI State: ${status.state}`);
  
  if (status.state !== "success") {
    console.log(`❌ CI not passed yet (${status.state}). Waiting for checks...`);
